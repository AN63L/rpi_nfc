...
def add_format_parser(parser):
    ...
    add_format_tt1_parser(subparsers.add_parser("tt1", description=description))
    add_format_tt2_parser(subparsers.add_parser("tt2", description=description))
    add_format_tt3_parser(subparsers.add_parser("tt3", description=description))
...
def add_format_tt1_parser(parser):
    ...
    parser.add_argument(
        "--tms", metavar="BYTE", type=parse_uint8, help="tag memory size, 8*(tms+1)"
    )
    parser.add_argument(
        "--rwa", metavar="BYTE", type=parse_uint8, help="read write access byte"
    )
...
def add_format_tt3_parser(parser):
    ...
    parser.add_argument(
        "--wf", metavar="BYTE", type=parse_uint8, help="write-flag attribute value"
    )
    parser.add_argument(
        "--rw", metavar="BYTE", type=parse_uint8, help="read-write flag attribute value"
    )
    parser.add_argument(
        "--len", metavar="INT", type=parse_uint24, help="ndef length attribute value"
    )
    parser.add_argument(
        "--crc", metavar="INT", type=parse_uint16, help="checksum attribute value"
    )
...
def add_emulate_parser(parser):
    ...
    parser.add_argument(
        "-l", "--loop", action="store_true", help="continue (restart) after tag release"
    )
    parser.add_argument(
        "-k", "--keep", action="store_true", help="keep tag memory (when --loop is set)"
    )
    parser.add_argument(
        "-s",
        dest="size",
    ...
    subparsers.required = True
    add_emulate_tt3_parser(subparsers.add_parser("tt3", help="emulate a type 3 tag"))
...
def add_emulate_tt3_parser(parser):
    parser.add_argument(
        "--idm",
        metavar="HEX",
        default="03FEFFE011223344",
        help="manufacture identifier (default: %(default)s)",
    )
    parser.add_argument(
        "--pmm",
        metavar="HEX",
        default="01E0000000FFFF00",
...
def add_protect_parser(parser):
    parser.add_argument("-p", dest="password", help="protect with password if possible")
    parser.add_argument(
        "--from",
        metavar="BLOCK",
        dest="protect_from",
    ...
    parser.add_argument(
        "--unreadable", action="store_true", help="make tag unreadable without password"
    )
...
class TagTool(cli.CommandLineInterface):
    def __init__(self):
        ...
        subparsers.required = True
        add_show_parser(subparsers.add_parser("show", help="pretty print ndef data"))
        add_dump_parser(subparsers.add_parser("dump", help="read ndef data from tag"))
        add_load_parser(subparsers.add_parser("load", help="write ndef data to tag"))
        add_format_parser(subparsers.add_parser("format", help="format ndef tag"))
        add_protect_parser(subparsers.add_parser("protect", help="write protect a tag"))
        add_emulate_parser(subparsers.add_parser("emulate", help="emulate an ndef tag"))
    ...
    def format_tag(self, tag):
        if not self.options.tagtype == "any":
            if not self.options.tagtype[2] == tag.type[4]:
                print(
                    "This is not a Type {0} Tag but you said so.".format(
                        self.options.tagtype[2]
                    )
                )
                return
        ...
        if formatted is True:
            {
                "tt1": self.format_tt1_tag,
                "tt2": self.format_tt2_tag,
                "tt3": self.format_tt3_tag,
                "tt4": self.format_tt4_tag,
                "any": lambda t: None,
            }[self.options.tagtype](tag)
    ...
    def protect_tag(self, tag):
        ...
        result = tag.protect(
            password, self.options.unreadable, self.options.protect_from
        )
        if result is True:
    ...
    def prepare_tt3_tag(self, target):
        if self.options.size % 16 != 0:
            ...
            log.warning(
                "tt3 ndef data area size rounded to {0}".format(self.options.size)
            )
        ...
        if not (hasattr(self.options, "tt3_data") and self.options.keep):
            if self.options.input:
                ...
                ndef_data_area = bytearray(self.options.data) + bytearray(
                    ndef_area_size - ndef_data_size
                )
            else:
    ...
    def emulate_tag_stop(self, tag):
        if self.options.preserve:
            ...
            log.info(
                "wrote tag memory to file '{0}'".format(self.options.preserve.name)
            )
