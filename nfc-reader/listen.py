...
def listen_ttf(timeout, clf, args):
    ...
    if target and target.tt3_cmd:
        if target.tt3_cmd[0] == 0x06:
            response = struct.pack("B", 29) + b"\7" + idm + b"\0\0\1" + bytearray(16)
...
def listen_dep(timeout, clf, args):
    ...
    target.atr_res = (
        b"\xD5\x01" + id3 + b"\0\0\0\x08" + (b"\x32" if gbt else b"\0") + gbt
    )
    ...
    if target and target.dep_req:
        ...
        logging.debug("send DEP_RES %s", hexlify(memoryview(dep_res)[1:]).decode())
        try:
        ...
        logging.debug("rcvd DEP_REQ %s", hexlify(memoryview(data)[1:]).decode())
